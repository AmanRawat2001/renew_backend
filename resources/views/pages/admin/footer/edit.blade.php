<x-layouts::app :title="__('Edit Footer Item')">
    <div class="flex h-full w-full flex-1 flex-col gap-4">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-semibold text-neutral-900 dark:text-neutral-50">{{ __('Edit Footer Item') }}
                </h1>
                <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
                    {{ __('Update footer item information') }}
                </p>
            </div>
            <a href="{{ route('admin.footers.index') }}" wire:navigate
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-800">
            <form action="{{ route('admin.footers.update', $footer) }}" enctype="multipart/form-data" method="POST"
                class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <input type="text" id="key" name="key"
                            ...
                            value="{{ old('key', $footer->key) }}" required />
                        @error('key')
                    ...
                    <div>
                        ...
                        <textarea id="value" name="value" rows="4"
                            ...
                            placeholder="{{ __('Enter footer item content') }}">{{ old('value', $footer->value) }}</textarea>
                        @error('value')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
                    ...
                    <div>
                        ...
                        <div class="flex flex-col gap-4">
                            ...
                            <div id="imagePreview" class="@if(!$footer->image) hidden @endif">
                                <p class="text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">{{ __('Current Image:') }}</p>
                                <div
                                    class="rounded-lg overflow-hidden border border-neutral-200 dark:border-neutral-700">
                                    <img id="previewImg" src="{{ $footer->image_url }}" alt="Preview"
                    ...
                    <div>
                        ...
                        <input type="number" id="sequence" name="sequence"
                            ...
                            placeholder="{{ __('e.g., 1') }}" value="{{ old('sequence', $footer->sequence) }}" min="0"
                            max="999" required />
                        @error('sequence')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" id="is_active" name="is_active" value="1"
                            ...
                            @if(old('is_active', $footer->is_active)) checked @endif />
                    ...
                    <div class="flex gap-3 pt-4">
                        <button type="submit"
                            ...
                            {{ __('Update Footer Item') }}
                        </button>
                        <a href="{{ route('admin.footers.index') }}" wire:navigate
                            class="inline-flex items-center gap-2 px-6 py-3 border border-neutral-300 dark:border-neutral-600 text-neutral-700 dark:text-neutral-300 font-medium rounded-lg hover:bg-neutral-50 dark:hover:bg-zinc-700 transition-colors">
                            {{ __('Cancel') }}
                        </a>
                    </div>
                </div>

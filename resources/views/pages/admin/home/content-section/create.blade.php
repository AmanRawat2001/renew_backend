<x-layouts::app :title="__('Create Content Section')">
    <div class="flex h-full w-full flex-1 flex-col gap-4">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-semibold text-neutral-900 dark:text-neutral-50">{{ __('Create Content Section') }}</h1>
                <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-1">{{ __('Add a new content section to your website') }}</p>
            </div>
            <a href="{{ route('admin.sections.index') }}" wire:navigate class="inline-flex items-center gap-2 px-4 py-2 text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-50 transition-colors">
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-800">
            <form action="{{ route('admin.sections.store') }}" method="POST" class="p-8">
                @csrf

                <div class="space-y-8">
                    <!-- Section Key Field -->
                    <div>
                        <label for="section_key" class="block text-sm font-semibold text-neutral-900 dark:text-neutral-50 mb-2">
                        ...
                        <input
                            ...
                            placeholder="{{ __('e.g., hero, about, vision, focus') }}"
                        ...
                        @enderror
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">{{ __('Unique identifier for this section (lowercase)') }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div wire:ignore>
                            <div id="quillTitleEditor" class="w-full rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-900 @error('title') border-red-500 @enderror" style="height: 120px;"></div>
                        </div>
                        <input type="hidden" id="title" name="title" value="{{ old('title') }}" required />
                        @error('title')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">{{ __('Rich HTML editor for section title') }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div wire:ignore>
                            <div id="quillSubtitleEditor" class="w-full rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-900 @error('subtitle') border-red-500 @enderror" style="height: 120px;"></div>
                        </div>
                        <input type="hidden" id="subtitle" name="subtitle" value="{{ old('subtitle') }}" />
                        @error('subtitle')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">{{ __('Rich HTML editor for section subtitle (optional)') }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div wire:ignore>
                            <div id="quillDescriptionEditor" class="w-full rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-900 @error('description') border-red-500 @enderror" style="height: 250px;"></div>
                        </div>
                        <input type="hidden" id="description" name="description" value="{{ old('description') }}" required />
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">{{ __('Rich HTML editor for detailed content') }}</p>
                    </div>
                </div>
                ...
                <div class="mt-8 flex items-center gap-3 pt-8 border-t border-neutral-200 dark:border-neutral-700">
                    ...
                    <a href="{{ route('admin.sections.index') }}" wire:navigate class="inline-flex items-center gap-2 px-6 py-2 text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-50 transition-colors">
                        {{ __('Cancel') }}
                    </a>
                </div>
            </form>

    ...
    <div class="flex h-full w-full flex-1 flex-col gap-4">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-1">{{ $impact->title }}</p>
            </div>
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-800">
            <form action="{{ route('admin.impacts.update', $impact) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            value="{{ old('metric_number', $impact->metric_number) }}"
                            required
                        />
                        @error('metric_number')
                    ...
                    <div>
                        ...
                        <input type="hidden" id="title" name="title" value="{{ old('title', $impact->title) }}"
                            required />
                        @error('title')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="hidden" id="description" name="description" value="{{ old('description', $impact->description) }}">
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="{{ __('0') }}"
                            value="{{ old('sequence', $impact->sequence) }}"
                            required
                        />
                        @error('sequence')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    ...
                    <div class="flex items-center gap-3">
                        <input
                            ...
                            name="down_arrow"
                            value="1"
                            {{ old('down_arrow', $impact->down_arrow) ? 'checked' : '' }}
                            class="w-4 h-4 rounded border-neutral-300 text-blue-600 focus:ring-blue-500"
                        />

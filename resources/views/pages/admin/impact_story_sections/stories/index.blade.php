<x-layouts::app :title="__('Impact Stories')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        ...
        <!-- Success Message -->
        @if (session('success'))
            <div class="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 rounded-lg">
                {{ session('success') }}
            </div>
        @endif
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-zinc-900">
                                <th class="px-4 py-3 text-left font-semibold text-neutral-900 dark:text-neutral-50">{{ __('Name') }}</th>
                                <th class="px-4 py-3 text-left font-semibold text-neutral-900 dark:text-neutral-50">{{ __('Section') }}</th>
                                <th class="px-4 py-3 text-left font-semibold text-neutral-900 dark:text-neutral-50">{{ __('Designation') }}</th>
                                <th class="px-4 py-3 text-left font-semibold text-neutral-900 dark:text-neutral-50">{{ __('Location') }}</th>
                                <th class="px-4 py-3 text-left font-semibold text-neutral-900 dark:text-neutral-50">{{ __('Sort Order') }}</th>
                        ...
                        <tbody>
                            @foreach ($stories as $story)
                                <tr class="border-b border-neutral-200 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-zinc-700/50">
                                    <td class="px-4 py-3 text-neutral-900 dark:text-neutral-50 max-w-xs truncate">{{ $story->name }}</td>
                                    <td class="px-4 py-3 text-neutral-600 dark:text-neutral-400">{{ $story->section->title ?? '—' }}</td>
                                    <td class="px-4 py-3 text-neutral-600 dark:text-neutral-400">{{ $story->designation }}</td>
                                    <td class="px-4 py-3 text-neutral-600 dark:text-neutral-400">{{ $story->location }}</td>
                                    <td class="px-4 py-3 text-neutral-600 dark:text-neutral-400">{{ $story->sort_order }}</td>
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <form action="{{ route('admin.impact_stories.destroy', $story) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50 rounded font-medium text-xs transition-colors"

<x-layouts::app :title="__('Create Slider')">
    <div class="flex h-full w-full flex-1 flex-col gap-4">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-semibold text-neutral-900 dark:text-neutral-50">{{ __('Create New Slider') }}
                </h1>
                <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
                    {{ __('Add a new slider to your homepage') }}</p>
            </div>
            <a href="{{ route('admin.main_sliders.index') }}" wire:navigate
        ...
        {{-- show all the error --}}
        @if ($errors->any())
            <div class="rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 p-4">
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-800">
            <form action="{{ route('admin.main_sliders.store') }}" enctype="multipart/form-data" method="POST"
                class="p-8">
                @csrf
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
                            {{ __('Rich HTML editor for formatting your  slider title') }}
                        </p>
                    </div>

                    <!-- Sub Title Field with Quill Editor -->
                    <div>
                        <label for="sub_title"
                            class="block text-sm font-semibold text-neutral-900 dark:text-neutral-50 mb-2">
                        ...
                        <div wire:ignore>
                            <div id="quillDescription"
                                class="w-full rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-900 @error('sub_title') border-red-500 @enderror"
                                style="height:150px;"></div>
                        </div>
                        <input type="hidden" id="sub_title" name="sub_title" value="{{ old('sub_title') }}" required />
                        @error('sub_title')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
                            {{ __('Rich HTML editor for formatting your slider description') }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="sequence" name="sequence" min="1"
                            ...
                            placeholder="{{ __('1') }}" value="{{ old('sequence') }}" required />
                        @error('sequence')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
                    ...
                    <div>
                        ...
                        <select id="page" name="page"
                            ...
                            @foreach (App\Enums\SliderPage::cases() as $pageCase)
        ...
        @push('scripts')
            <script>
                ...
                function initQuill() {
                    ...
                    let oldTitle = @json(old('title'));

                    if (oldTitle) {
                        window.quill.clipboard.dangerouslyPasteHTML(oldTitle);
                    }
                ...
                function initQuillDescription() {
                    ...
                    const editor = document.querySelector('#quillDescription');

                    if (!editor) return;

                    if (editor.classList.contains("ql-container")) return;

                    window.quillDescription = new Quill('#quillDescription', {
                        theme: 'snow',
                        modules: {
                            toolbar: [
                                [{
                    ...
                    let oldDescription = @json(old('sub_title'));

                    if (oldDescription) {
                        window.quillDescription.clipboard.dangerouslyPasteHTML(oldDescription);
                    }

                    window.quillDescription.on('text-change', function() {
                        document.getElementById('sub_title').value = window.quillDescription.root.innerHTML;
                    });
                }

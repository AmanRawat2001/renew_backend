            ...
            <form
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <input type="hidden" id="title" name="title" value="{{ old('title', $main_slider->title) }}"
                            required />
                        @error('title')
                    ...
                    <div>
                        ...
                        <input type="hidden" id="sub_title" name="sub_title"
                            value="{{ old('sub_title', $main_slider->sub_title) }}" required />
                        @error('sub_title')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
                    ...
                    <div>
                        ...
                        <input type="number" id="sequence" name="sequence" min="1"
                            ...
                            placeholder="{{ __('1') }}" value="{{ old('sequence', $main_slider->sequence) }}"
                            required />
                        @error('sequence')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        ...
                        <select id="page" name="page" disabled
                            ...
                            @foreach (App\Enums\SliderPage::cases() as $pageCase)
                                <option value="{{ $pageCase->value }}"
                                    {{ old('page', $main_slider->page->value) === $pageCase->value ? 'selected' : '' }}>
                                    {{ $pageCase->label() }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" id="is_active" name="is_active" value="1"
                            {{ old('is_active', $main_slider->is_active) ? 'checked' : '' }}
        ...
        @push('scripts')
            <script>
                ...
                function initQuillDescription() {
                    ...
                    let descriptionValue = document.getElementById('sub_title').value;

                    if (descriptionValue) {
                        window.quillDescription.clipboard.dangerouslyPasteHTML(descriptionValue);
                    }

<x-layouts::app :title="__('Edit Mission Slide')">
    <div class="flex h-full w-full flex-1 flex-col gap-4">
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.other_mission_sliders.index') }}" wire:navigate class="inline-flex items-center gap-2 px-4 py-2 text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-50 transition-colors">
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-800">
            <form action="{{ route('admin.other_mission_sliders.update', $other_mission_slider) }}" method="POST" enctype="multipart/form-data" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-8">
                        ...
                        @if($other_mission_slider->image)
                            <div class="mt-4">
                                ...
                                <div class="rounded-lg overflow-hidden max-w-sm">
                                    <img src="{{ asset('uploads/' . $other_mission_slider->image) }}" alt="{{ $other_mission_slider->title }}" class="w-full h-48 object-cover" />
                                </div>
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        <input
                            ...
                            value="{{ old('title', $other_mission_slider->title) }}"
                            required
                        />
                    ...
                    <div>
                        ...
                        <input type="hidden" id="description" name="description" value="{{ old('description', $other_mission_slider->description) }}" required />
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            value="{{ old('external_link', $other_mission_slider->external_link) }}"
                        />
                        @error('external_link')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
                            {{ __('Optional URL link for this mission slide') }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="{{ __('0') }}"
                            value="{{ old('sequence', $other_mission_slider->sequence) }}"
                            required
                        />
                        @error('sequence')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        ...
                        <select id="page" name="page"
                            ...
                            @foreach (App\Enums\SitePage::cases() as $pageCase)
                                <option value="{{ $pageCase->value }}"
                                    {{ old('page', $other_mission_slider->page->value) === $pageCase->value ? 'selected' : '' }}>
                                    {{ $pageCase->label() }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center gap-3">
                        <input
                            ...
                            name="is_active"
                            value="1"
                            {{ old('is_active', $other_mission_slider->is_active) ? 'checked' : '' }}
                ...
                <div class="mt-8 flex items-center gap-3 pt-8 border-t border-neutral-200 dark:border-neutral-700">
                    ...
                    <a href="{{ route('admin.other_mission_sliders.index') }}" wire:navigate class="inline-flex items-center gap-2 px-6 py-2 text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-50 transition-colors">
                        {{ __('Cancel') }}
                    </a>
                </div>
            </form>
